// viz_tuition_earnings.js
(function () {

    window.VizTuitionEarnings = {
      ...
      draw: function (p, manager) {
        ...
        p.textSize(18);
        p.text("Median Post-Enrollment Earnings vs. Average Tuition", 42, 38);
        ...
        if (!manager._tuitionEarnData) {
          manager._tuitionEarnData = manager.scorecardRows
            .map(function (d) { return { name: d.name, earn10: d.earn10, avgCost: d.avgCost }; })
            .filter(function (d) { return d.earn10 != null && d.avgCost != null; });
        }
        ...
        p.textAlign(p.CENTER);
        p.textSize(14);
        p.text("Average Tuition (dollars)", offsetX + w / 2, offsetY + h - 60);
        p.push();
        ...
        for (var k = 0; k < data.length; k++) {
          ...
          if (d < hoverDist) {
            hoverDist = d;
            hover = { x: px, y: py, name: data[k].name, avgCost: data[k].avgCost, earn10: data[k].earn10 };
          }
        }
        ...
        for (var r = 0; r < n; r++) {
          sumX += data[r].avgCost;
          sumY += data[r].earn10;
          sumXY += data[r].avgCost * data[r].earn10;
